...
public class WarehouseManager {
	...
	public static void main(String[] args) {
		...
		do {
			for (int i = gusu; i < ABKosanArray1.length; i++) {
				if (ABKosanArray1[i] % 2 == 0) {
					gusu = i;
					break;
				}
			}
			for (int i = kisu; i < ABKosanArray2.length; i++) {
				if (ABKosanArray2[i] % 2 != 0) {
					int trade = ABKosanArray1[gusu];
					ABKosanArray1[gusu] = ABKosanArray2[i];
					ABKosanArray2[i] = trade;
					kisu = i;
					flag++;
					break;
				}
